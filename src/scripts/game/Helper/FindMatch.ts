//9 = magic number; row, column length
export class FindMatch {
  ...
  private depthFirstSearch(i: number, j: number, currentLabel: number, group: any) {
    if (i < 0 || i == 9) return;
    if (j < 0 || j == 9) return;
    if (this._symbolLabelArray[i][j] || group != this._cloneSymbolTypeArray[i][j]) return;
    this._symbolLabelArray[i][j] = currentLabel;
    for (var direction = 0; direction < this.directionX.length; ++direction) {
  ...
  public getResult(result: any[][]) {
    for (var i = 0; i < 9; i++) {
      ...
      for (var j = 0; j < 9; j++) {
        this._cloneSymbolTypeArray[i][j] = result[i][j].type;
