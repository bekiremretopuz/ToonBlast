...
export class MediaInfoViewer {
  ...
  public show(): void {
    this.rootContainer.className = this.rootContainer.className
      .replace("hidden", "")
      .trim();
  }
  ...
  private getText(): string {
    const stageInitialGCD = Maths.gcd(
      this.data.display.stage.initialWidth,
      this.data.display.stage.initialHeight
    );
    return (
      `` +
      ...
      `<div>initial ${this.data.display.stage.initialWidth}x${
        this.data.display.stage.initialHeight
      } ${this.data.display.stage.initialWidth / stageInitialGCD}:${
        this.data.display.stage.initialHeight / stageInitialGCD
      }</div>` +
